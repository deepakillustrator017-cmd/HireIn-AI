document.addEventListener("DOMContentLoaded", async function () {
  ...
  form.addEventListener("submit", async function(event){
    ...
    try {
      ...
      if(ai){try{var review=await ai.request("analyzeATS",{resume:resumeText.slice(0,18000),jobDescription:jobText.slice(0,12000)});if(Number.isFinite(Number(review.score)))score=Math.max(0,Math.min(100,Math.round(Number(review.score))));if(Array.isArray(review.matchedSkills))matched=review.matchedSkills.slice(0,30);if(Array.isArray(review.missingKeywords))missing=review.missingKeywords.slice(0,12);if(Array.isArray(review.formattingIssues))formatting=review.formattingIssues.slice(0,8);if(Array.isArray(review.suggestions))tips=review.suggestions.slice(0,8);}catch(e){/* The keyword score remains available when AI is not configured. */}}
      ...
      var saved=await client.from("ats_history").insert({user_id:user.id,job_id:jobIdValue,resume_name:file.name,resume_path:path,score:score,matched_keywords:matched,missing_keywords:missing,suggestions:tips.join(" "),details:{formatting_issues:formatting}});
